(function($){
  ...
  function allowedMessage(){
    return 'Only PNG or PSD allowed for original art.';
  }
  ...
  function clearStoredFile($label){
    $label.find('.bb-file-pill').text('').removeAttr('data-url').hide();
    $label.find('.bb-upload-original').removeData('bb-url').removeData('bb-id');
  ...
  function openPicker(slug){
    ...
    frame.on('select', function(){
      ...
      if (allowed.indexOf(ext) === -1) { clearStoredFile($label); showError($label, allowedMessage()); return; }
      clearInline($label);
  ...
  $(document).on('change', '#bb_production', enforceProductionForAll);
