(function($){
  'use strict';
  var rasterOriginals = ['png'];
  var vectorOriginals = ['svg', 'ai', 'eps'];
  ...
  function isScreenPrintProduction(){
    return String($('#bb_production').val() || '').trim().toLowerCase() === 'screen print';
  }

  function allowedOriginals(){
    return isScreenPrintProduction() ? vectorOriginals : rasterOriginals;
  }
  ...
  function originalArtExtFromAttachment(file){
    if (file && file.mime) {
      ...
      if (mime === 'application/illustrator' || mime === 'application/vnd.adobe.illustrator' || mime === 'application/ai') return 'ai';
      if (mime === 'application/eps' || mime === 'application/x-eps' || mime === 'image/eps' || mime === 'image/x-eps') return 'eps';
    }
    return extOf((file && (file.filename || file.name || file.url)) || '');
  }

  function allowedMessage(){
    return isScreenPrintProduction()
      ? 'Only SVG, AI, or EPS allowed for original art.'
      : 'Only PNG allowed for original art.';
  }
  ...
  function currentArtUrl($row){
    return String($row.find('.bb-art-url').val() || '').trim();
  }

  function currentArtExt($row){
    var fileName = String($row.find('.bb-art-filename').text() || '').trim();
    return extOf(fileName || currentArtUrl($row));
  }

  function clearArtSelection($row){
    $row.find('.bb-art-url').val('');
    $row.find('.bb-art-id').val('');
    setPreview($row, '', '');
  }

  function enforceProductionForRow($row){
    var url = currentArtUrl($row);
    var ext;
    if (!url) {
      clearError($row);
      return;
    ...
    ext = currentArtExt($row);
    if (ext && allowedOriginals().indexOf(ext) === -1){
      clearArtSelection($row);
      showError($row, allowedMessage());
      return;
    }
