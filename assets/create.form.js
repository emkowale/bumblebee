(function($){
  ...
  function selectedSizes(){
    return $('.bb-size-checkbox:checked').map(function(){
  ...
  function validateSizes(){
    ...
    if (selectedSizes().length === 0){
      show($el, BumblebeeCreate.required.sizes);
      return false;
    }
    clearInline($el);
    return true;
  ...
  function renderColorRows(count){
    ...
    for(let i=0;i<count;i++){
      ...
      const row = [
        ...
        '  <div class="bb-color-row__field">',
        '    <label>Mockup Image <span class="bb-tooltip dashicons dashicons-editor-help" aria-label="Choose a mockup image that has a garment in this color. Use WebP when possible, or upload a PNG/JPG and Bumblebee will convert it to WebP." title="Choose a mockup image that has a garment in this color. Use WebP when possible, or upload a PNG/JPG and Bumblebee will convert it to WebP."></span></label>',
  ...
  $('#bb_price,#bb_title,#bb_color_count,#bb_production,#bb_special_instructions').on('input change', function(){
    clearInline($(this));
    if(this.id==='bb_price') requirePrice();
  });
  $(document).on('change', '.bb-size-checkbox', function(){
    validateSizes();
  });
  ...
  $('#bb_create_btn').on('click', function(e){
    ...
    fd.append('sizes', selectedSizes().join(','));
