<?php
/* JSON-mode OpenAI client. Key from secret file / const / env / option (in that order). */
if (!defined('ABSPATH')) exit;

function bee_ai_key(){
  $f = __DIR__.'/secret_key.php';
  if (file_exists($f)) { $k = @include $f; if (is_string($k) && $k!=='') return $k; }
  if (defined('BUMBLEBEE_OPENAI_KEY') && BUMBLEBEE_OPENAI_KEY) return BUMBLEBEE_OPENAI_KEY;
  $env = getenv('OPENAI_API_KEY'); if ($env) return $env;
  return (string) get_option('bumblebee_ai_key','');
}

/**
 * Call OpenAI in strict JSON mode.
 * Ensures messages include the word "json" (lowercase) to satisfy API requirement.
 */
...
/* Lightweight connectivity check for Settings */
