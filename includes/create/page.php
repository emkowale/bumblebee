...
function bumblebee_render_create_page(){
  ...
  $bb_default_sizes = ['XS', 'S', 'M', 'L', 'XL', '2XL', '3XL', '4XL', '5XL', '6XL'];
  ?>
  ...
  <table class="form-table" role="presentation"><tbody>
    ...
    <tr>
      ...
      <td>
        <div id="bb_sizes" class="bb-size-grid" role="group" aria-label="Garment Size(s)">
          <?php foreach ($bb_size_options as $bb_size_option): ?>
            <label class="bb-size-option" for="bb_size_<?php echo esc_attr(sanitize_key($bb_size_option)); ?>">
              <input
                type="checkbox"
                class="bb-size-checkbox"
                id="bb_size_<?php echo esc_attr(sanitize_key($bb_size_option)); ?>"
                value="<?php echo esc_attr($bb_size_option); ?>"
                <?php checked(in_array($bb_size_option, $bb_default_sizes, true)); ?>
              />
              <span><?php echo esc_html($bb_size_option); ?></span>
            </label>
          <?php endforeach; ?>
        </div>
        <div class="bb-inline-error" data-for="bb_sizes"></div>
      </td>
    </tr>
    <tr>
      <th>
...
<?php endfor; ?>
    ...
    <tr>
      <th><label for="bb_production">Production</label></th>
      <td>
        <select id="bb_production" required>
          <option value="">— Select —</option>
          <?php foreach (function_exists('bumblebee_production_options') ? bumblebee_production_options() : ['DTG', 'DTF', 'Screen Print', 'Embroidery', 'UV', 'Fulfill'] as $bb_production_option): ?>
            <option><?php echo esc_html($bb_production_option); ?></option>
          <?php endforeach; ?>
        </select>
        <div class="bb-inline-error" data-for="bb_production"></div>
      </td>
    </tr>
    <tr>
