...
class BB_AI_Content {
  public static function generate(array $args): array {
    ...
    $spent = bb_ai_get_month_spend();
    $budget = bb_ai_monthly_budget();
    if ($spent >= $budget) {
      $month = bb_ai_month_key();
      $msg = sprintf('AI monthly budget of $%.2f reached for %s.', $budget, $month);
      return ['ok'=>false,'data'=>null,'error'=>$msg,'provider'=>null];
    }
    ...
    foreach ($keys as $i => $key) {
      ...
      if (!$res['ok']) {
        ...
        $last_error = isset($res['error']) ? (string)$res['error'] : 'AI request failed.';
      ...
      if (isset($res['json']['usage']) && is_array($res['json']['usage'])) {
        $cost = bb_ai_cost_from_usage($res['json']['usage'], $model);
        if ($cost > 0) bb_ai_add_spend($cost);
      }
      return ['ok'=>true,'data'=>$clean,'provider'=>$i===0?'primary':'secondary','error'=>null];
    }
    return ['ok'=>false,'data'=>null,'error'=>$last_error !== '' ? $last_error : 'AI request failed (keys exhausted or invalid response)','provider'=>null];
  }
}
