...
function bb_render_bumblebee_product_tab(){
  ...
  $base_price = (string) get_post_meta($post->ID, 'Bumblebee Base Price', true);
  if ($base_price === '' && function_exists('bumblebee_soundwave_infer_base_price') && function_exists('bumblebee_soundwave_collect_variation_rows')) {
    $product = wc_get_product($post->ID);
    if ($product instanceof WC_Product) {
      $inferred_base_price = bumblebee_soundwave_infer_base_price(
        bumblebee_soundwave_collect_variation_rows($product)
      );
      if ($inferred_base_price > 0) {
        $base_price = wc_format_decimal($inferred_base_price, wc_get_price_decimals());
      }
    }
  }
  ?>
  <div id="bb_bumblebee_product_data" class="panel woocommerce_options_panel bb-bumblebee-panel">
    <div class="options_group">
      ...
      <p class="form-field">
        <label for="bb_price">Price</label>
        <input type="number" min="0" step="0.01" id="bb_price" name="bb_price" value="<?php echo esc_attr($base_price); ?>" />
      </p>
...
add_action('woocommerce_process_product_meta', function($post_id){
  ...
  $base_price_raw = isset($_POST['bb_price']) ? wc_format_decimal(wp_unslash($_POST['bb_price']), wc_get_price_decimals()) : '';
  $has_base_price = ($base_price_raw !== '' && is_numeric($base_price_raw) && (float) $base_price_raw >= 0);
  ...
  if ($has_base_price) {
    update_post_meta($post_id, 'Bumblebee Base Price', $base_price_raw);
    if (function_exists('bumblebee_soundwave_apply_base_price')) {
      bumblebee_soundwave_apply_base_price($post_id, (float) $base_price_raw);
    }
  } else {
    delete_post_meta($post_id, 'Bumblebee Base Price');
  }
});
