...
function bb_render_bumblebee_product_tab(){
  ...
  $is_fulfill = function_exists('bumblebee_is_fulfill_production')
    ? bumblebee_is_fulfill_production($production)
    : (strcasecmp($production, 'Fulfill') === 0);
  ?>
        ...
        <span class="bb-print-locations">
          <?php foreach ($bb_locations as $slug => $label): ?>
            <?php
              ...
              if ($art_id > 0 && bb_original_art_attachment_is_allowed($art_id, $production) && function_exists('bumblebee_resolve_original_art_url')) {
                $original_art_url = bumblebee_resolve_original_art_url($art_id);
...
add_action('woocommerce_process_product_meta', function($post_id){
  ...
  if (!empty($print_locs)) {
    foreach ($print_locs as $loc) {
      ...
      if ($aid > 0) {
        if (bb_original_art_attachment_is_allowed($aid, $production)) {
          $aid_url = function_exists('bumblebee_resolve_original_art_url') ? bumblebee_resolve_original_art_url($aid) : '';
          if ($aid_url) $url = esc_url_raw($aid_url);
        } else {
          $url = '';
        }
      }
      if ($url !== '' && bb_original_art_url_is_allowed($url, $production)) {
