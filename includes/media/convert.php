 ...
 */
function bumblebee_convert_and_rename_to_webp( $attachment_id, $company_name, $product_title, array $args = [] ) {
  ...
  if ( $ext === 'webp' ) return $attachment_id;
  ...
  if ( is_wp_error( $editor ) ) {
    if ( ! empty($args['allow_original_fallback']) ) {
  ...
  if ( ! empty($args['delete_original']) && (int) $new_id !== (int) $attachment_id ) {
...
function bumblebee_safe_rename_original($attachment_id, $base_raw, $base_slug, $dir, $ext) {
  ...
  if ( @rename( get_attached_file( $attachment_id, true ), $target_path ) ) {
