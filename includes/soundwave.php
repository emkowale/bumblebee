...
function bumblebee_soundwave_large_size_pricing_state(array $rows): array {
  ...
  foreach ($rows as $row) {
    ...
    $expected = (float) wc_format_decimal($base_price * (1 + $large_markups[$size]), $decimals);
...
function bumblebee_soundwave_fix_large_size_pricing(int $product_id): array {
  ...
  foreach ($rows as $row) {
    ...
    $expected = (string) wc_format_decimal($base_price * (1 + $large_markups[$size]), $decimals);
    $actual = isset($row['price']) ? (float) $row['price'] : 0.0;
...
function bumblebee_soundwave_status(int $product_id): array {
  ...
  $is_fulfill = strcasecmp((string) $production, 'Fulfill') === 0;

  foreach ($meta_keys as $label => $meta_key) {
    if ($meta_key === 'Print Location' && $is_fulfill) {
      $checks[] = ['label'=>$label, 'ok'=>true];
      continue;
    }
  ...
  if ($is_fulfill) {
    $checks[] = ['label'=>'Original Art (per Print Location)','ok'=>true];
  } elseif (!empty($print_locations)) {
