...
function bumblebee_hub_get_vendors() {
  ...
  if (is_array($terms)) {
    foreach ($terms as $t) {
      ...
      $name_val = isset($t['name']) ? (string) $t['name'] : '';
      $name_val = html_entity_decode($name_val, ENT_QUOTES | ENT_HTML5, 'UTF-8');
      $code_val = html_entity_decode((string) $code_val, ENT_QUOTES | ENT_HTML5, 'UTF-8');
      $desc_val = html_entity_decode((string) $desc_val, ENT_QUOTES | ENT_HTML5, 'UTF-8');
      $out[] = [
