...
function bumblebee_production_options(): array {
  return ['DTG', 'DTF', 'Screen Print', 'Embroidery', 'UV', 'Fulfill'];
}

function bumblebee_normalize_production_name(string $production): string {
  return strtolower(trim($production));
}

function bumblebee_is_fulfill_production(string $production): bool {
  return bumblebee_normalize_production_name($production) === 'fulfill';
}

function bumblebee_is_screen_print_production(string $production): bool {
  return bumblebee_normalize_production_name($production) === 'screen print';
}

function bumblebee_allowed_original_art_extensions(string $production): array {
  if (bumblebee_is_screen_print_production($production)) {
    return ['svg', 'ai', 'eps'];
  }
  return ['png'];
}

function bumblebee_original_art_requirement_label(string $production): string {
  if (bumblebee_is_screen_print_production($production)) {
    return 'vector files (.SVG, .AI, or .EPS)';
  }
  return 'PNG files';
}

function bumblebee_attachment_extension_from_mime(string $mime): string {
  switch (strtolower(trim($mime))) {
    case 'image/png':
      return 'png';
    case 'image/svg+xml':
      return 'svg';
...
function bumblebee_is_allowed_original_art_extension(string $ext, string $production): bool {
  $ext = strtolower(trim($ext));
  if ($ext === '') return false;
  return in_array($ext, bumblebee_allowed_original_art_extensions($production), true);
}

function bumblebee_original_art_attachment_extension(int $attachment_id): string {
  if ($attachment_id <= 0) return '';

  $ext = function_exists('bumblebee_attachment_extension_from_id')
    ? strtolower(trim((string) bumblebee_attachment_extension_from_id($attachment_id)))
    : '';
  if ($ext !== '') return $ext;

  return bumblebee_attachment_extension_from_mime((string) get_post_mime_type($attachment_id));
}

function bumblebee_is_allowed_original_art_attachment(int $attachment_id, string $production): bool {
  if ($attachment_id <= 0) return false;

  $ext = bumblebee_original_art_attachment_extension($attachment_id);
  if ($ext !== '') {
    return bumblebee_is_allowed_original_art_extension($ext, $production);
  }

  // Unknown metadata: do not hard-fail here; downstream URL validation can still succeed.
  return true;
}

function bumblebee_resolve_original_art_url(int $attachment_id): string {
  if ($attachment_id <= 0) return '';

  if (function_exists('bumblebee_original_attachment_url')) {
    $url = bumblebee_original_attachment_url($attachment_id);
...
function bumblebee_invalid_art_urls(array $art_urls, string $production, array $art_ids_by_slug = []): array {
  $bad = [];
  foreach ($art_urls as $art) {
    ...
    if ($att_id > 0) {
      $ext = bumblebee_original_art_attachment_extension($att_id);
      if ($ext !== '') {
        if (!bumblebee_is_allowed_original_art_extension($ext, $production)) {
          $bad[] = bumblebee_art_label_from_slug($slug);
        }
        continue;
      }
      if (!bumblebee_is_allowed_original_art_attachment($att_id, $production)) {
        $bad[] = bumblebee_art_label_from_slug($slug);
      }
    }

    $ext = bumblebee_extension_from_url($url);
    if (!bumblebee_is_allowed_original_art_extension($ext, $production)) {
      $bad[] = bumblebee_art_label_from_slug($slug);
    }
  }
...
function bumblebee_parse_create_request(): array {
  ...
  $is_fulfill = bumblebee_is_fulfill_production($prod);
  ...
  $invalid_art = bumblebee_invalid_art_urls($art_urls, $prod, $art_ids_by_slug);
  if (!empty($invalid_art)) {
    $msg = 'Original Art must be ' . bumblebee_original_art_requirement_label($prod) . '. Please update: ' . implode(', ', $invalid_art) . '.';
