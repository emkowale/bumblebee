...
function bumblebee_maybe_generate_ai_content($product_id, string $vendor_code, string $image_url): array {
  try {
    ...
    if ( file_exists($__bb_root.'includes/ai.php') ) {
      ...
      $err = is_array($ai) && isset($ai['error']) ? (string)$ai['error'] : 'AI response was invalid.';
      return ['ok'=>false,'error'=>$err];
    }
    return ['ok'=>false,'error'=>'AI module is missing.'];
  } catch (\Throwable $e) {
    // swallow; product creation must succeed
    $msg = $e->getMessage();
    return ['ok'=>false,'error'=> $msg !== '' ? $msg : 'AI error'];
  }
