...
function bumblebee_size_price_surcharge(string $size): float {
  ...
  return isset($surcharge_map[$normalized]) ? (float) $surcharge_map[$normalized] : 0.0;
}

function bumblebee_variation_price_for_size(float $base_price, string $size): string {
  $surcharge = bumblebee_size_price_surcharge($size);
  $adjusted_price = $base_price + $surcharge;
...
function bumblebee_build_product(array $req): array {
  ...
  bumblebee_attach_art_meta($product_id, $req['locations'], $req['production']);
