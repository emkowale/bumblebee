...
function bumblebee_build_product(array $req): array {
  ...
  if (!empty($req['turbo_rip_url'])) {
    $product->update_meta_data('Turbo RIP File', $req['turbo_rip_url']);
  }
  if (!empty($req['turbo_rip_id'])) {
    wp_update_post([
      'ID'          => $req['turbo_rip_id'],
      'post_parent' => $product_id,
    ]);
  }
  if (!empty($req['embroidery_file_url'])) {
    $product->update_meta_data('Embroidery File', $req['embroidery_file_url']);
  }
  if (!empty($req['embroidery_file_id'])) {
    wp_update_post([
      'ID'          => $req['embroidery_file_id'],
      'post_parent' => $product_id,
    ]);
  }
