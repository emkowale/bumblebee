...
function bumblebee_attach_art_meta($product_id, array $loc_opts, string $production = ''){
  foreach ($loc_opts as $loc) {
    ...
    if ($aid > 0) {
      if (!bumblebee_is_allowed_original_art_attachment($aid, $production)) {
        $resolved_url = '';
      } else {
        $aid_url = bumblebee_resolve_original_art_url($aid);
        if ($aid_url) $resolved_url = esc_url_raw($aid_url);
      }
    } elseif ($resolved_url !== '') {
      $ext = function_exists('bumblebee_extension_from_url') ? bumblebee_extension_from_url($resolved_url) : '';
    ...
    if (isset($_POST[$id_key])) {
      ...
      if ($att_id > 0) {
        if (!bumblebee_is_allowed_original_art_attachment($att_id, $production)) {
          ...
          $att_url = bumblebee_resolve_original_art_url($att_id);
