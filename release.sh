...
OWNER="emkowale"
REPO="bumblebee"
PLUGIN_SLUG="bumblebee"      # folder name inside the zip
MAIN_FILE="bumblebee.php"    # main plugin file (repo root or under PLUGIN_SLUG/)
# ==============================================================================
...
# --- GitHub release (optional, non-fatal) ------------------------------------
if command -v gh >/dev/null 2>&1; then
  step "GitHub release v${NEXT}"

  if gh release view "v${NEXT}" >/dev/null 2>&1; then
    # Try to upload; on failure just warn and move on
    if ! gh release upload "v${NEXT}" "${ART}/${ZIP}" --clobber >/dev/null 2>&1; then
      warn "gh release upload failed (auth/perms?). Zip is ready at ${ART}/${ZIP}."
    else
      ok "Updated existing GitHub release v${NEXT}"
    fi
  else
    # Try to create; on failure just warn and move on
    if ! gh release create "v${NEXT}" "${ART}/${ZIP}" -t "v${NEXT}" -n "Release ${NEXT}" >/dev/null 2>&1; then
      warn "gh release create failed (auth/perms?). Zip is ready at ${ART}/${ZIP}."
    else
      ok "Created GitHub release v${NEXT}"
    fi
  fi
else
  warn "gh not installed; skipped GitHub release (zip is at ${ART}/${ZIP})."
fi
