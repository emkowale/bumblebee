#!/usr/bin/env bash
# Bumblebee release script — safe PHP updater, friendly status, self-healing git.
set -euo pipefail

# ==== CONFIG ==================================================================
OWNER="emkowale"
REPO="bumblebee"
PLUGIN_SLUG="bumblebee"       # top-level folder name inside the zip
MAIN_FILE="bumblebee.php"     # plugin main file (your repo keeps it at root)
...
SKIP_GITHUB_RELEASE="${BB_SKIP_GITHUB_RELEASE:-0}"
GIT_OK=1
...
if [[ "$SKIP_GITHUB_RELEASE" == "1" ]]; then
  warn "GitHub release publishing disabled via BB_SKIP_GITHUB_RELEASE=1"
elif [[ "$GIT_OK" -eq 0 ]]; then
  warn "Skipping GitHub release preflight (git unavailable)"
else
  command -v gh >/dev/null 2>&1 || die "gh not found. Install GitHub CLI or run with BB_SKIP_GITHUB_RELEASE=1"
...
# ==== GITHUB RELEASE ===========================================================
if [[ "$SKIP_GITHUB_RELEASE" == "1" ]]; then
  warn "Skipped GitHub release (BB_SKIP_GITHUB_RELEASE=1)"
elif [[ "$GIT_OK" -eq 0 ]]; then
